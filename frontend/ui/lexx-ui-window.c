...
#include "lexx-ui-error.h"
#include <epoxy/gl.h>
#include <math.h>

enum { X_AXIS, Y_AXIS, Z_AXIS, N_AXES };

struct _LexxUiWindow
{
...
struct _LexxUiWindowClass
{
  GtkApplicationWindowClass parent_class;
};

G_DEFINE_TYPE (LexxUiWindow, lexx_ui_window, GTK_TYPE_APPLICATION_WINDOW)
  ...
  if (status == GL_FALSE)
    {
      ...
      g_set_error (error, LEXX_UI_ERROR, LEXX_UI_ERROR_SHADER_COMPILATION,
  ...
  if (status == GL_FALSE)
    {
      ...
      g_set_error (error, LEXX_UI_ERROR, LEXX_UI_ERROR_SHADER_LINK,
...
gl_init (LexxUiWindow *self)
{
  ...
  title = g_strdup_printf ("lexx on %s", renderer ? renderer : "Unknown");
  gtk_window_set_title (GTK_WINDOW (self), title);
  g_free (title);
}

static void
gl_fini (LexxUiWindow *self)
{
  /* we need to ensure that the GdkGLContext is set before calling GL API */
  gtk_gl_area_make_current (GTK_GL_AREA (self->gl_drawing_area));
...
static void
draw_triangle (LexxUiWindow *self)
{
...
static void
adjustment_changed (LexxUiWindow *self,
                    GtkAdjustment   *adj)
{
...
static void
animate_toggled (LexxUiWindow *self)
{
...
static void
lexx_ui_window_class_init (LexxUiWindowClass *klass)
{
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  gtk_widget_class_set_template_from_resource (widget_class, "/ui/frontend/lexx/lexx-ui-window.ui");

  gtk_widget_class_bind_template_child (widget_class, LexxUiWindow, gl_drawing_area);
  gtk_widget_class_bind_template_child (widget_class, LexxUiWindow, x_adjustment);
  gtk_widget_class_bind_template_child (widget_class, LexxUiWindow, y_adjustment);
  gtk_widget_class_bind_template_child (widget_class, LexxUiWindow, z_adjustment);
  gtk_widget_class_bind_template_child (widget_class, LexxUiWindow, animate_button);
...
static void
lexx_ui_window_init (LexxUiWindow *self)
{
  ...
  gtk_window_set_icon_name (GTK_WINDOW (self), "lexx");
}

GtkWidget *
lexx_ui_window_new (LexxUi *app)
{
  return g_object_new (lexx_ui_window_get_type (), "application", app, NULL);
}
